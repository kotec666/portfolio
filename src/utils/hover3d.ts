...
export default function hover3d(ref: React.RefObject<HTMLDivElement>, { x = 0, y = 0, z = 0 }) {
    const [coords, setCoords] = useState({ x: 0, y: 0 });
    const [isHovering, setIsHovering] = useState(false);

    const handleMouseMove = (e: MouseEvent) => {
        const { offsetWidth: width, offsetHeight: height } = ref?.current as HTMLDivElement
